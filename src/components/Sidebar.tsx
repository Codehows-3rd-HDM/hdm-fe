...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="fixed left-0 top-0 w-[260px] h-screen bg-white border-r border-[#e0e0e0] 
                    font-[Malgun_Gothic,'Apple_SD_Gothic_Neo',sans-serif] flex flex-col overflow-y-auto">
      {/* 로고 영역 */}
      ...
      {menuItems.map((depth1) => {
        ...
        return (
          <div key={depth1.title}>
            {/* --- Level 1 --- */}
            <div
              className={`flex items-center px-6 mb-3 cursor-pointer text-base transition-all
            ...
            >
              {Icon && <Icon size={20} strokeWidth={1.5} className="mr-10" />}
              <span className="block leading-[2.2rem] mr-10" style={{paddingLeft: '7px'}}>{depth1.title}</span>
              {hasSub1 && (
                isClosed1
                  ? <ChevronDown size={16} className="ml-auto text-[#999] rotate-180" />
                  : <ChevronDown size={16} className="ml-auto text-[#999]" />
              )}
            </div>
                ...
                {depth1.items!.map((depth2) => {
                  ...
                  return (
                    <div key={depth2Key}>
                      {/* --- Level 2 --- */}
                      <div
                        className={`flex items-center pr-6 mb-2 cursor-pointer text-sm transition-colors
                          ${isHighlight2 ? 'text-[#007bff] font-semibold bg-[#eef6ff]' : 'text-[#555] font-normal'}
                        `}
                        style={{ paddingLeft: '40px',paddingRight: '10px', height: '45px' }}
                        onClick={() => handleToggleDepth2(depth2Key, hasSub2, depth2.path)}
                      >
                        <span className={`mr-2 text-[18px] ${isHighlight2 ? 'text-[#007bff]' : 'text-[#b0b0b0]'}`}>~</span>
                        <span className="block leading-[2rem]" style={{paddingLeft: '7px'}}>{depth2.title}</span>
                        {hasSub2 && (
                          isClosed2
                            ? <ChevronDown size={14} className="ml-auto text-[#bbb] rotate-180" />
                            : <ChevronDown size={14} className="ml-auto text-[#bbb]" />
                        )}
                      </div>
                        ...
                        >
                          {depth2.items!.map((depth3) => {
                            ...
                            return (
                              <div
                                key={depth3.title}
                                className={`flex items-center pr-6 mb-2 cursor-pointer text-[13px]
                              ...
                              >
                                <Hexagon size={12} className={`mr-2 ${isActive3 ? 'fill-[#007bff]' : 'fill-[#ddd]'} stroke-none`} />
                                <span className="block leading-[1.8rem]" style={{paddingLeft: '4px'}}>{depth3.title}</span>
                              </div>
                            );
                          })}
                        </div>
