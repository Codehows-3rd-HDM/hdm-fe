...
const ActivityListTemplate: React.FC<ActivityListTemplateProps> = ({ isAdmin }) => {
  ...
  const handleExcelDownload = () => {
    ...
    const rows = filteredActivities
      .map(
        item =>
          `${item.id},"${item.title}",${item.startDate},${item.endDate},${item.cost},"${item.effect}","${item.content.replace(
            /"/g,
            '""'
          )}"`
      )
    ...
    <div className="px-6 py-4">
      ...
      <div className="bg-white shadow-sm rounded-lg p-5 mb-6 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-700">활동 기간 :</span>
        </div>
      ...
      <div className="flex justify-center mt-12">
        <div className="flex gap-2 text-sm text-gray-500">
          ...
          <span className="px-3 py-1 font-bold border-b-2 border-gray-800 text-gray-900">1</span>
